...
export function OrderDetailsSidebar({
  ...
  useEffect(() => {
    ...
    const loadOrderDetails = async () => {
      ...
      setOrder(null);
      try {
        const orderDetails = await apiClient.fetchOrderById(orderId);
        setOrder(orderDetails);

        const allMenus = await apiClient.fetchMenus({});
  ...
  const handleAcceptOrder = async () => {
    // Use the correct property: internalId
    if (!order || typeof order.internalId !== "number") {
      Alert.alert("Error", "Order ID is missing or invalid.");
      return;
    }
    setIsActionLoading(true);
    try {
      await apiClient.updateOrderStatus(order.internalId, "preparing"); // Use 'preparing' or your desired next status
  ...
  const handleRejectOrder = async () => {
    ...
    setIsActionLoading(true);
    try {
      await apiClient.updateOrderStatus(order.internalId, "cancelled");
          ...
          <View className="gap-y-3 p-4 rounded-lg border border-border">
            <View className="flex-row justify-between items-center">
              ...
              <Badge className={`${statusStyles.bg} border-0`}>
                <Text
                  className={`font-bold text-sm capitalize ${statusStyles.text}`}
                >
                  {order.status}
                </Text>
              </Badge>
